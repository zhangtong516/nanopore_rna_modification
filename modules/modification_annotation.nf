process MODIFICATION_ANNOTATION {
    ...
    output:
    tuple val(samplename), path("${samplename}_modifications.anno.bed.gz"), emit: modifications_anno_bed
    ...
    ${projectDir}/bin/process_modkit_out.sh \
        -c 1 -r 0.0 \
        ${modifications_bed} \
        ${samplename}_modifications.anno.bed.gz \
        ${task.cpus}
    ...
    if [ -s ${modifications_bed} ]; then
        echo "Total modification sites: \$(zcat ${modifications_bed} | wc -l)" >> ${samplename}_modification_summary.txt
        zcat ${modifications_bed} | awk 'BEGIN{OFS="\t"} {mod_type=\$4; freq[mod_type]++} END{for(m in freq) print m, freq[m]}' | \ 
